                    ...
                    <div class="card">
                        <div class="header">
                            <a class="btn btn-xs btn-info waves-effect"
                                href="{{ route($ParentRouteName) }}">{{ __('root.common.all') }}  ({{ $total_products }})</a>
                            <a @if ($trash_show == 0) class="dis-none" @endif
                                class="btn btn-xs btn-danger waves-effect text-black"
                                href="{{ route($ParentRouteName . '.trashed') }}">{{ __('root.common.trash') }} ({{ $total_trashed_products }})
                            </a>
                                    ...
                                    <table class="table table-hover table-bordered table-sm">
                                        <thead>
                                            <tr>
                                                ...
                                                <th  class="width-160 text-center">{{ __('root.product.options') }}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <?php $i = 1; ?>
                                            @foreach ($items as $item)
                                                <tr>
                                                    ...
                                                    <td class="text-bold">
                                                        {{ Helper::convertMoneyFormat($item->total_flat_price) }}</td>
                                                    <td>{{ Helper::convertMoneyFormat($item->car_parking_charge) }}</td>
                                                    <td>{{ Helper::convertMoneyFormat($item->utility_charge) }}</td>
                                                    <td>{{ Helper::convertMoneyFormat($item->additional_work_charge) }}
                                                    </td>
                                                    <td>{{ Helper::convertMoneyFormat($item->other_charge) }}</td>
                                                    <td>{{ Helper::convertMoneyFormat($item->discount_or_deduction) }}</td>
                                                    <td>{{ Helper::convertMoneyFormat($item->refund_additional_work_charge) }}
                                                    </td>
                                                    <td class="text-bold">
                                                        {{ Helper::convertMoneyFormat($item->net_sells_price) }}</td>
